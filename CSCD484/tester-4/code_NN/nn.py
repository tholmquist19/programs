...
class NeuralNetwork:
        ...
        for _ in range(iterations):
            if SGD:
                ...
                for i in range(0, n, mini_batch_size):
                    xBatch = x_shuffle[i:i + mini_batch_size]
                    yBatch = y_shuffle[i:i + mini_batch_size]
                    self._fit_batch(xBatch, yBatch, eta)
            else:
    ...
    def _fit_batch(self, X, Y, eta):
        ...
        for l in range(1, len(self.layers)):
            A_withBias = np.concatenate((A[-1], np.ones((A[-1].shape[0], 1))), axis=1)
            a = (A_withBias @ self.layers[l].W)
            A.append(mu.MyMath.tanh(a))
        ...
        for l in range(self.L, 0, -1):
            delta[l - 1] = mu.MyMath.tanh_de(A[l - 1]) * (delta[l] @ self.layers[l].W[1:, :].T)

        # Update weights
        for l in range(1, len(self.layers)):
            ones = np.ones((A[l - 1].shape[0], 1))
            A_withBias = np.concatenate((ones, A[l - 1]), axis=1)
            self.layers[l].W -= eta * (A_withBias.T @ delta[l]) / n
    ...
    def predict(self, X):
        ...
        A = X
        for l in range(1, len(self.layers)):
            A = mu.MyMath.tanh((np.hstack([A, np.ones((A.shape[0], 1))]) @ self.layers[l].W))

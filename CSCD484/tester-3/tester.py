    ...
    for lam in [0,10]:
        for z_r in [1,2]:
            for eta in [0.01,0.001]:
                ...
                (exp_train_error,mikes_test_error) = errors[row]
                row+=1
                if not inThreshold(train_error,exp_train_error) or not inThreshold(test_error,mikes_test_error):
                    print(f'For GD, and the following params:\nlam: {lam}, z_r: {z_r}, eta: {eta}')
                    print(f'Expected train/test error:\n{exp_train_error}, {mikes_test_error}')
